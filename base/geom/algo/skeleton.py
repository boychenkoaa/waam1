...
from base.geom.primitives.linear import Polygon, BypasDirection
...
def _normalize_contour(contour):
    ...
    return [point for prev, point, next in _window(contour) if
            not (point == next or (point - prev).normalized() == (next - point).normalized())]
...
class _SplitEvent(namedtuple("_SplitEvent", "distance, intersection_point, vertex, opposite_edge")):
    ...
    def __str__(self):
        return "{} Split event @ {} from {} to {}".format(self.distance, self.intersection_point, self.vertex,
                                                          self.opposite_edge)
...
class _EdgeEvent(namedtuple("_EdgeEvent", "distance intersection_point vertex_a vertex_b")):
    ...
    def __str__(self):
        return "{} Edge event @ {} between {} and {}".format(self.distance, self.intersection_point, self.vertex_a,
                                                             self.vertex_b)
            ...
            for edge in self.original_edges:
                ...
                if i is not None and not _approximately_equals(i, self.point):
                    ...
                    xleft = _cross(edge.bisector_left.v.normalized(),
                                   (b - edge.bisector_left.p).normalized()) > -EPSILON
                    xright = _cross(edge.bisector_right.v.normalized(),
                                    (b - edge.bisector_right.p).normalized()) < EPSILON
...
def gg_skeleton(polygons: list, need_clipping: bool):
    gg_list = []
    for polygon in polygons:
        contours = polygon.get_all_contours()
        poly_exterior = contours[0].points(BypasDirection.CCW)
        holes = []
        for hole in contours[1:]:
            holes.append(hole.points(BypasDirection.CW))

        skeleton = skeletonize(poly_exterior, holes)
        segment_list = []
        for arc in skeleton:
            for sink in arc.sinks:
                segment_list.append(((arc.source.x, arc.source.y), (sink.x, sink.y)))
        result_geom_graph = GeomGraph(segment_list)
        if need_clipping:
            result_geom_graph.clipping(1)
        gg_list.append(result_geom_graph)
    return gg_list
